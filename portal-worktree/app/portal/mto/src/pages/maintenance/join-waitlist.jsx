        ...
        <Stack sx={{ gap: { xs: 4, sm: 5 }, m: { xs: 2.5, sm: 3 }, alignItems: 'center', justifyContent: 'center' }}>
          <Stack sx={{ gap: { xs: 1.5, sm: 2.5 }, alignItems: 'center', justifyContent: 'center' }}>
            <Typography sx={{ fontSize: { xs: 30, sm: 45, md: 50, lg: 54 }, fontWeight: 700, textAlign: 'center', lineHeight: 1.2 }}>
              <FormattedMessage id="maintenance.waitlist.title" />
            </Typography>
            <Typography sx={{ fontSize: { xs: 16, sm: 18, lg: 20 }, maxWidth: 508, color: 'text.secondary', textAlign: 'center' }}>
              <FormattedMessage id="maintenance.waitlist.body" />
            </Typography>
          </Stack>

          <OutlinedInput
            type="email"
            placeholder={intl.formatMessage({ id: 'maintenance.emailPlaceholder' })}
            fullWidth
            sx={{
          ...
          <Stack sx={{ gap: 1.25, justifyContent: 'center' }}>
            <AvatarGroup max={6}>
              {[avatar5, avatar6, avatar7, avatar8, avatar9, avatar10].map((src, idx) => (
                <Avatar key={idx} src={src} alt={intl.formatMessage({ id: 'maintenance.waitlist.memberAlt' }, { index: idx + 1 })} />
              ))}
            </AvatarGroup>
            <Typography sx={{ color: 'text.secondary', textAlign: 'center' }}>
              <FormattedMessage id="maintenance.waitlist.count" />
            </Typography>
          </Stack>
        </Stack>
      </Stack>
