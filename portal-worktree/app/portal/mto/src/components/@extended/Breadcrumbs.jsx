...
export default function Breadcrumbs({
  ...
  const getLabel = (label) => (isFormatted && label ? <FormattedMessage id={label} /> : (label ?? ''));
          ...
          >
            <MuiBreadcrumbs aria-label="breadcrumb" maxItems={maxItems || 8} separator={separatorIcon}>
              <Typography component={Link} to="/" variant="h6" sx={{ color: 'text.secondary', textDecoration: 'none' }}>
                ...
                {(!icon || icons) && getLabel('breadcrumb.home')}
              </Typography>
              {mainContent}
            </MuiBreadcrumbs>
            {title && titleBottom && (
      ...
      breadcrumbContent = (
        ...
        >
          <Stack
            ...
            sx={{ gap: 1, justifyContent: rightAlign ? 'space-between' : 'flex-start', alignItems: rightAlign ? 'center' : 'flex-start' }}
          >
            {title && !titleBottom && <Typography variant="h2">{getLabel(custom ? heading : item?.title)}</Typography>}
            {tempContent}
            {title && titleBottom && (
              <Typography variant="h2" sx={{ mt: card === false ? 0.25 : 1 }}>
                {getLabel(custom ? heading : item?.title)}
              </Typography>
            )}
          </Stack>
          {card === false && divider !== false && <Divider sx={{ mt: 2 }} />}
