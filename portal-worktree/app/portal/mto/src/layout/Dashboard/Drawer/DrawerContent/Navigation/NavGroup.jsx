...
export default function NavGroup({
  ...
  const navCollapse = item.children?.map((menuItem, index) => {
    switch (menuItem.type) {
      ...
      default:
        return (
          <Typography key={index} variant="h6" sx={{ color: 'error.main', textAlign: 'center' }}>
            <FormattedMessage id="nav.error.groupChildren" />
          </Typography>
        );
    }
  });
    ...
    <Fragment key={i}>
      ...
      {itemRem?.elements?.map((menu) => {
        switch (menu.type) {
          ...
          default:
            return (
              <Typography key={menu.id} variant="h6" sx={{ color: 'error.main', textAlign: 'center' }}>
                <FormattedMessage id="nav.error.menuItems" />
              </Typography>
            );
        }
      })}
        ...
        <List>
          ...
          >
            ...
            <ListItemText
              sx={{ mr: 1 }}
              primary={
                <Typography variant="body1" sx={{ color: isSelected || anchorEl ? 'primary.main' : 'secondary.dark' }}>
                  <FormattedMessage id={currentItem.id === lastItemId || !currentItem?.title ? 'more-items' : currentItem?.title} />
                </Typography>
              }
            />
            {openMini ? (
