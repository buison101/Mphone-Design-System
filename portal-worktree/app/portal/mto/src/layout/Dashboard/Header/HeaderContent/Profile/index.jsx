...
export default function Profile() {
  ...
  return (
    <Box sx={{ flexShrink: 0, ml: 'auto' }}>
      <Tooltip title={intl.formatMessage({ id: 'profile.title' })} disableInteractive>
        <ButtonBase
          sx={(theme) => ({
            p: 0.25,
            borderRadius: 1,
              ...
              <ClickAwayListener onClickAway={handleClose}>
                <MainCard elevation={0} border={false} content={false}>
                    ...
                    <Stack direction="row" sx={{ justifyContent: 'space-between', alignItems: 'center' }}>
                      {user && (
                        <Stack direction="row" sx={{ gap: 1.25, alignItems: 'center' }}>
                          <Avatar alt={user.name} src={user.avatar ?? avatar1} sx={{ width: 32, height: 32 }} />
                          <Stack>
                      ...
                      <Tooltip title={intl.formatMessage({ id: 'profile.logout' })}>
                  ...
                  <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                    <Tabs variant="fullWidth" value={value} onChange={handleChange} aria-label={intl.formatMessage({ id: 'profile.tabs' })}>
                      <Tab
                        sx={{
                          display: 'flex',
                          flexDirection: 'row',
                        ...
                        icon={<UserOutlined />}
                        label={intl.formatMessage({ id: 'profile.title' })}
                        {...a11yProps(0)}
                      />
                      <Tab
                        sx={{
