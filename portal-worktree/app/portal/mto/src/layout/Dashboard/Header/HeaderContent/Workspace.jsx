...
const colors = {
  'workspace.plan.basic': 'default',
  'workspace.plan.pro': 'primary'
};

// ==============================|| WORKSPACE - LABEL ||============================== //

function Label({ title, sx }) {
  return <Chip label={<FormattedMessage id={title} />} size="small" color={colors[title]} sx={{ height: 20, ...sx }} />;
}
...
export default function Workspace() {
  ...
  return (
    <Box>
      ...
      >
        <Avatar
          alt={intl.formatMessage({ id: workspace.title })}
          variant="rounded"
        ...
        {!downSM && (
          <>
            <Box component="span" sx={{ typography: 'h6', whiteSpace: 'nowrap' }}>
              <FormattedMessage id={workspace.title} />
            </Box>
