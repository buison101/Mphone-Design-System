  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      ...
      const result = searchData
        .map((group) => {
          const matchedChilds = group.childs.filter((child) => intl.formatMessage({ id: child.title }).toLowerCase().includes(lower));
          ...
          {filteredData.length > 0 ? (
                      ...
                      <Grid key={index} size={{ xs: 6, sm: 4, md: 3 }}>
                        <MainCard content={false} sx={{ ':hover': { borderColor: 'primary.main' } }}>
                          ...
                          >
                            ...
                            <FormattedMessage id={childItem.title} />
                          </Stack>
                        </MainCard>
                      </Grid>
                    ))}
            ...
            <Stack sx={{ gap: 1, textAlign: 'center', py: 6 }}>
              <Typography variant="h6">
                <FormattedMessage id="header.search.empty.title" />
              </Typography>
              <Typography variant="body2" sx={{ color: 'text.secondary' }}>
                <FormattedMessage id="header.search.empty.hint" />
              </Typography>
            </Stack>
          )}
        </DialogContent>
