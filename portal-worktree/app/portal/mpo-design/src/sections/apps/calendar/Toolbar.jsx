...
export default function Toolbar({ date, view, onClickNext, onClickPrev, onClickToday, onChangeView }) {
  ...
  const [viewFilter, setViewFilter] = useState(getViewOptions());

  useEffect(() => {
    if (downSM) {
      const filter = getViewOptions().filter((item) => item.value !== 'dayGridMonth' && item.value !== 'timeGridWeek');
      setViewFilter(filter);
    } else {
      setViewFilter(getViewOptions());
    }
  }, [downSM]);
