...
export default function CustomizedAutocomplete() {
  const { getRootProps, getInputProps, getItemProps, getListboxProps, getOptionProps, groupedOptions, value, focused, setAnchorEl } =
    useAutocomplete({
  ...
  const customAutocompleteCodeString = `// CustomizedAutocomplete.jsx
<Root>
  <div {...getRootProps()}>
    <InputWrapper ref={setAnchorEl} className={focused ? 'focused' : ''}>
      {value.map((option: FilmOptionType, index: number) => (
        <StyledTag label={option.label} {...getItemProps({ index })} />
      ))}
      <input {...getInputProps()} />
    </InputWrapper>
  </div>
        ...
        <div {...getRootProps()}>
          <InputWrapper ref={setAnchorEl} className={focused ? 'focused' : ''}>
            {value.map((option, index) => (
              <StyledTag label={option.label} {...getItemProps({ index })} key={index} />
            ))}
            <input {...getInputProps()} />
          </InputWrapper>
        </div>
