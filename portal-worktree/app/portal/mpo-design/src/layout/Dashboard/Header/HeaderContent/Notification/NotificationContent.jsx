...
// project imports
import { getNotificationData, getNotificationFilterOptions } from './data';
...
export default function NotificationContent({ notifications, filter, setFilter, onMarkAllRead }) {
  return (
    <MainCard
      ...
      content={false}
      secondary={
        <Link href="#" underline="hover" sx={{ typography: 'caption', textTransform: 'none' }} onClick={onMarkAllRead}>
          Make all as read
        </Link>
      }
    >
      ...
      >
        {getNotificationFilterOptions().map((opt, index) => (
          <Chip
            key={index}
            label={opt.label}
            ...
            {...{
              ...(opt.key === 'all' && {
                avatar: <Avatar size="xs">{getNotificationData().length}</Avatar>
              })
            }}
          />
        ))}
