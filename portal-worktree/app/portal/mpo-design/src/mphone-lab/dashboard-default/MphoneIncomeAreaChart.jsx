...
function Legend({ items, onToggle }) {
  return (
    <Stack direction="row" sx={{ gap: 2, alignItems: 'center', justifyContent: 'center', mt: 2.5, mb: 1.5 }}>
      {items.map((item) => (
        <Stack key={item.id} direction="row" sx={{ gap: 1.25, alignItems: 'center', cursor: 'pointer' }} onClick={() => onToggle(item.id)}>
...
export default function IncomeAreaChart({ seriesLabels = ['Page views', 'Sessions'], view }) {
  const theme = useTheme();

  // Keyed by series id, not by label. The label is display text and is
  // translated at build time (docs/15 §4.7).
...
IncomeAreaChart.propTypes = { seriesLabels: PropTypes.arrayOf(PropTypes.node), view: PropTypes.oneOf(['monthly', 'weekly']) };
