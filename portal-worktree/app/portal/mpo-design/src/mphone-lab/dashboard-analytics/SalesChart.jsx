...
export default function SalesChart({ content = {}, filter = 'today' }) {
  const theme = useTheme();
  const downSM = useMediaQuery(theme.breakpoints.down('sm'));

  // Keyed by series id, not by label. The label is display text and is
  // translated at build time (docs/15 §4.7); keying state by it made every
  // series invisible in Vietnamese and left highlightedItem pointing at a
  // series that no longer existed.
  const [seriesVisibility, setSeriesVisibility] = useState({ Income: true, CostOfSales: true });
  const [highlightedItem, setHighlightedItem] = useState({ seriesId: 'Income' });

  const toggleSeriesVisibility = (seriesId) => {
    setSeriesVisibility((prev) => {
      const isNowHidden = prev[seriesId];
      if (isNowHidden && highlightedItem?.seriesId === seriesId) {
        setHighlightedItem(null);
      }
      return { ...prev, [seriesId]: !prev[seriesId] };
    });
  };
  ...
  const valueFormatter = (value) => `${content.valuePrefix ?? '$ '}${value}${content.valueSuffix || ' Thousands'}`;
  ...
  const chartData = useMemo(() => {
    ...
    switch (filter) {
      case 'month':
        labels = content.weekLabels || ['Week 1', 'Week 2', 'Week 3', 'Week 4'];
        ...
        break;
      case 'year':
        labels = content.monthLabels || ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  ...
  }, [content.monthLabels, content.weekLabels, filter]);

  const { labels, incomeData, income2Data, cosData, cos2Data } = chartData;

  const initialSeries = [
    { id: 'Income', data: incomeData, stack: 'income', label: content.primary || 'Income', color: warningColor, valueFormatter },
    { id: 'Income2', data: income2Data, stack: 'income', label: content.primary || 'Income', color: warningLightColor, valueFormatter },
    { id: 'CostOfSales', data: cosData, stack: 'cos', label: content.secondary || 'Cost of Sales', color: primaryColor, valueFormatter },
    {
      id: 'CostOfSales2',
      data: cos2Data,
      stack: 'cos',
      label: content.secondary || 'Cost of Sales',
      ...
      <Box sx={{ p: 2.5, pb: 0 }}>
        <Stack direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
          <Box>
            <Typography gutterBottom sx={{ color: 'text.secondary', fontSize: 14 }}>
              {content.summaryLabel || 'Net Profit'}
            </Typography>
            <Typography variant="h4">{content.summaryValue || '$1560'}</Typography>
          </Box>
            ...
            {initialSeriesCopy.map((series) => (
              <Stack
                ...
                sx={{
                  gap: 1,
                  alignItems: 'center',
                  opacity: seriesVisibility[series.id] ? 1 : 0.45,
                  cursor: 'pointer',
        ...
        <BarChart
          ...
          series={initialSeries
            .map((series) => ({ ...series, type: 'bar', color: withAlpha(series.color, 0.85), visible: seriesVisibility[series.id] }))
...
SalesChart.propTypes = { content: PropTypes.object, filter: PropTypes.any };
