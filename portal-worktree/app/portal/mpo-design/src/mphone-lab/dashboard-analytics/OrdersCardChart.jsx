...
export default function OrdersCardChart({ label = 'Orders' }) {
  ...
  return (
    <LineChart
      ...
      series={[{ type: 'line', label, data, showMark: false, area: true, id: 'Order', color: theme.vars.palette.error.main }]}
      slotProps={{ tooltip: { trigger: 'axis', sx: { caption: { display: 'none' } } } }}
      sx={{
        [`& .${lineClasses.line}`]: { strokeWidth: 1.5 },
        [`& .${lineClasses.area}`]: { fill: "url('#orderGradient')" }
      }}
    >
      <defs>
...
OrdersCardChart.propTypes = { label: PropTypes.node };
