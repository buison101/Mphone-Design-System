...
export default function WelcomeBanner({ content = {} }) {
  return (
    <MainCard
      border={false}
      sx={(theme) => ({
    ...
    >
      <Grid container>
        <Grid size={{ md: 6, sm: 6, xs: 12 }}>
          ...
          >
            <Typography variant="h2">{content.title || 'Welcome to Mantis'}</Typography>
            ...
            <Box>
              ...
              >
                {content.action || 'View full statistic'}
              </Button>
            </Box>
          </Stack>
        </Grid>
        <Grid sx={{ display: { xs: 'none', sm: 'initial' } }} size={{ sm: 6, xs: 12 }}>
          <Stack sx={{ alignItems: 'flex-end', justifyContent: 'center', position: 'relative', pr: { sm: 3, md: 8 } }}>
            <CardMedia component="img" src={WelcomeImage} alt={content.imageAlt || 'Welcome'} sx={{ width: 'auto' }} />
            <Box sx={{ position: 'absolute', bottom: 0, right: '10%' }}>
              <CardMedia component="img" src={WelcomeImageArrow} alt={content.arrowAlt || 'Welcome Arrow'} />
            </Box>
          </Stack>
        </Grid>
      </Grid>
...
WelcomeBanner.propTypes = { content: PropTypes.object };
