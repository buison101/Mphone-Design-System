...
export default function EnhancedTable() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelectedId = getRows().map((n) => n.name);
  ...
  const handleClick = (event, name) => {
    ...
    const selectedRowData = getRows().filter((row) => newSelected.includes(row.name.toString()));
  ...
  // avoid a layout jump when reaching the last page with empty getRows().
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - getRows().length) : 0;
  ...
  return (
    <MainCard
      ...
      title="Data Table"
      secondary={
        <CSVExport data={selectedValue.length > 0 ? selectedValue : getRows()} headers={getHeader()} filename={'selected-table-data.csv'} />
      }
    >
