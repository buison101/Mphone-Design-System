  ...
  return (
    <Grid container spacing={{ xs: 3, md: 5 }}>
      ...
      <Grid size={{ xs: 12, sm: 9, md: 10 }}>
        <Stack sx={{ gap: 5 }}>
          {banner && (
            <CardMedia sx={{ width: { xs: 1, md: '70%' }, borderRadius: { xs: 2, sm: 3, md: 5 } }} image={banner} component="img" />
          )}
...
export default function ChangeLog() {
  ...
  useEffect(() => {
    startTransition(async () => {
      try {
        const response = await axiosServices.get('api/change-log/react/mantis');
