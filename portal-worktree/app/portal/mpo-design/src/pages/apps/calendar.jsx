...
export default function Calendar() {
  // FullCalendar writes its own day names, button text and empty state; none of
  // that goes through the keymap. Its `vi` locale covers the words, and the
  // narrow weekday width is what gives `CN`, `T2`, `T3` instead of `Thứ 2`.
  // English keeps the vendor default on both.
  const { state } = useConfig();
  const isVietnamese = state.i18n === 'vi';
  ...
  return (
    <Box sx={{ position: 'relative' }}>
      <CalendarStyled>
        ...
        <FullCalendar
          locale={isVietnamese ? viLocale : undefined}
          dayHeaderFormat={isVietnamese ? { weekday: 'narrow' } : undefined}
          weekends
          editable
          droppable
          selectable
