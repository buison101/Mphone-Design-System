...
export default defineConfig(({ mode }) => {
  ...
  const API_URL = env.VITE_APP_BASE_NAME || '/p/';
  const PORT = 3000;
  ...
  return {
    ...
    build: {
      // built straight into the document root so nginx serves /p/ as static files
      outDir: path.resolve(import.meta.dirname, '../../../p'),
      emptyOutDir: true,
