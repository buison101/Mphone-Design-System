...
export default function CallHistoryPhase1() {
  ...
  const query = useMemo(
    () => ({ ...filters, q: debouncedSearch, page: page + 1, page_size: pageSize }),
    [filters, debouncedSearch, page, pageSize]
  );
  const { data, isLoading, isValidating, error, forbidden, refresh } = useCallHistory(query);
  ...
  return (
    <Grid container spacing={2.5}>
      <Grid size={12}>
        <PageHeader
          title={<FormattedMessage id="history.title" />}
          description={<FormattedMessage id="history.description" />}
          actions={
            <>
      ...
      <Grid size={12}>
        <MainCard contentSX={{ p: 2 }}>
          <Grid container spacing={1.5} sx={{ alignItems: 'center' }}>
            <Grid size={{ xs: 12, md: 4 }} sx={{ order: 1 }}>
              <TextField
                fullWidth
                size="small"
                value={filters.q}
            ...
            <Grid size={{ xs: 6, md: 2 }} sx={{ order: 2 }}>
              <TextField
                fullWidth
                size="small"
                type="date"
            ...
            {extensions.length > 1 && (
              <Grid size={{ xs: 6, md: 2 }} sx={{ display: showAdvanced ? 'block' : 'none', order: 10 }}>
                <TextField
                  fullWidth
                  select
                  size="small"
            ...
            <Grid size={{ xs: 6, md: 2 }} sx={{ order: 3 }}>
              <TextField
                fullWidth
                size="small"
                type="date"
            ...
            <Grid size={{ xs: 12, md: 2 }} sx={{ order: 4 }}>
              <Button
                fullWidth
                size="small"
                variant={advancedFilterCount > 0 ? 'outlined' : 'text'}
                aria-expanded={showAdvanced}
                onClick={() => setShowAdvanced((value) => !value)}
              >
                <FormattedMessage
                  id={showAdvanced ? 'filter.hideAdvanced' : 'filter.showAdvanced'}
                  values={{ count: advancedFilterCount }}
                />
              </Button>
            </Grid>
            <Grid size={{ xs: 12, md: 2 }} sx={{ order: 5 }}>
              <Button fullWidth size="small" color="inherit" disabled={!hasFilters || isValidating} onClick={reset}>
                <FormattedMessage id="action.resetFilters" />
              </Button>
            </Grid>
            {isValidating && data && (
              <Grid size={{ xs: 12, md: 2 }} sx={{ order: 12 }} role="status" aria-live="polite">
                <Typography variant="caption" color="text.secondary">
                  <FormattedMessage id="filter.updating" />
                </Typography>
              </Grid>
            )}
          </Grid>
      ...
      <Grid size={12}>
        <MainCard content={false}>
          ...
          <DataTableContainer ariaLabel={intl.formatMessage({ id: 'history.title' })}>
          ...
          <DataPagination
            ...
            total={total}
            busy={isLoading}
            pageSizeOptions={CALL_PAGE_SIZES}
