...
export default function ActiveCalls() {
  ...
  const [pendingHangup, setPendingHangup] = useState('');
  ...
  return (
    <Grid container spacing={2.5}>
      <Grid size={12}>
        <PageHeader title={<FormattedMessage id="calls.title" />} description={<FormattedMessage id="calls.description" />} />
      </Grid>
      ...
      <Grid size={12}>
        <MainCard content={false}>
          <DataTableContainer ariaLabel={intl.formatMessage({ id: 'calls.title' })}>
            <Table size="small" sx={{ minWidth: 720 }}>
              ...
              <TableBody>
                {calls.map((call) => (
                  <CallRow key={call.unique_id} call={call} canHangup={canHangup} onHangup={setPendingHangup} now={now} />
                ))}
                {calls.length === 0 && (
                  <TableRow>
      ...
      <ConfirmActionDialog
        open={Boolean(pendingHangup)}
        title={<FormattedMessage id="calls.hangupConfirmTitle" />}
        description={<FormattedMessage id="calls.hangupConfirmDescription" />}
        cancelLabel={<FormattedMessage id="action.cancel" />}
        confirmLabel={<FormattedMessage id="calls.hangup" />}
        onCancel={() => setPendingHangup('')}
        onConfirm={() => {
