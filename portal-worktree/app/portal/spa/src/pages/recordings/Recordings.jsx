...
const initialFilters = { from: initialFrom, to: today, q: '', page: 1, page_size: 20 };
...
export default function Recordings() {
  ...
  const queryFilters = { ...filters, q: debouncedSearch };
  const { data, error, isLoading, isValidating, refresh } = useAnalytics(RECORDINGS_URL, queryFilters);
  ...
  return (
    <Grid container spacing={2.5}>
      <Grid size={12}>
        <PageHeader title={<FormattedMessage id="recordings.title" />} description={<FormattedMessage id="recordings.description" />} />
      </Grid>
      ...
      <Grid size={12}>
        <MainCard content={false}>
          <FilterBar
            ...
            resetDisabled={filters.from === initialFrom && filters.to === today && filters.q === ''}
          ...
          {isLoading && !data ? (
            ...
            <DataTableContainer ariaLabel={intl.formatMessage({ id: 'recordings.title' })}>
          ...
          <DataPagination
            ...
            total={data?.totals?.recordings ?? 0}
            busy={isValidating}
