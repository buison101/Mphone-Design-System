...
export default function useCallHistory(filters) {
  ...
  const { data, error, isLoading, isValidating, mutate } = useSWR(`${CALLS_URL}?${query.toString()}`, fetcher, {
    keepPreviousData: true,
    revalidateOnFocus: false
  });

  return { data, error, isLoading, isValidating, refresh: mutate, forbidden: Boolean(error?.forbidden) };
}
