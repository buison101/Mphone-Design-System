...
| `src/index.jsx` | Imports `src/styles/mphone-owner-overrides.css` after every other global stylesheet | Gives the product owner one protected, final CSS override layer for the whole Portal | Re-add the import after all vendor and font stylesheet imports; keep it as the final stylesheet import |
| `index.html` | Adds the `emotion-insertion-point` marker before application styles | Keeps runtime-generated MUI/Emotion rules ahead of the product-owner override stylesheet in the document cascade | Re-add the named meta element as the first child of `<head>` |
| `src/components/RTLLayout.jsx` | Anchors both LTR and RTL Emotion caches to the shared insertion point | Prevents either direction cache from appending generated MUI styles after the product-owner override stylesheet | Resolve the named meta element once and pass it as `insertionPoint` to both caches |
