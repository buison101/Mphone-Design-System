...
| `src/layout/Dashboard/Header/HeaderContent/Localization.jsx` | Added a Vietnamese entry to the language switcher | Vietnamese is the default locale, so the switcher has to offer it — without it a user who picks English cannot get back | Re-add the one `ListItemButton` block, copying the shape of its neighbours |
