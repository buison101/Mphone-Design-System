...
//the browser follows redirects, so answer with a status the application can act on
	portal_identity_validate_session(true);
...
//refresh newly installed dialer permissions for existing signed-in sessions
	require_once dirname(__DIR__, 2) . '/auto_dialer/resources/auto_dialer_auth.php';
...
//only the permissions the interface actually branches on are sent to the browser
	...
	$entitlements = [];
	if (portal_identity_has_workspace()) {
		foreach (['call_recording', 'speech_to_text', 'ai_summary', 'call_statistics', 'auto_dialer'] as $capability_name) {
			$entitlements[$capability_name] = portal_entitlement_decision($capability_name, false);
		}
...
//the websocket router is only proxied on the tls listener
	...
	echo json_encode([
		...
		'customer' => portal_identity_has_workspace() ? [
			'customer_uuid' => $_SESSION['portal_identity']['customer_uuid'] ?? null,
		] : null,
