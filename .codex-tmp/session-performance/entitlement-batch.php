<?php
	...
	function portal_entitlement_decision(string $capability, bool $validate_identity = true): array {
		if (($validate_identity && !portal_identity_validate_session(true)) || !portal_identity_has_workspace()) {
	...
	function portal_entitlement_decisions(array $capabilities, bool $validate_identity = true): array {
		$capabilities = array_values(array_unique(array_filter(array_map('strval', $capabilities))));
		$denied = ['allowed' => false, 'effective_allowed' => false, 'enforcement_mode' => 'enforce'];
		$decisions = array_fill_keys($capabilities, $denied);
		if (empty($capabilities)
			|| ($validate_identity && !portal_identity_validate_session(true))
			|| !portal_identity_has_workspace()) {
			return $decisions;
		}
		$result = customer_platform_request([
			'action' => 'portal_entitlement_check',
		...
		foreach ($capabilities as $capability) {
			$source = is_array($payload['decisions'][$capability] ?? null) ? $payload['decisions'][$capability] : [];
			$decisions[$capability] = [
				'allowed' => ($source['allowed'] ?? false) === true,
				'effective_allowed' => ($source['effective_allowed'] ?? false) === true,
				'enforcement_mode' => (string) ($source['enforcement_mode'] ?? 'enforce'),
